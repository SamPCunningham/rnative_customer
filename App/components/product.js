...
function Items({ done: doneHeading, onPressItem }) {
  ...
  const heading = doneHeading ? "Completed" : "Product Name";
...
export default function Product({ navigation }) {
  ...
  return (
    <View style={styles.container}>
      <Text>Product Information Listed here</Text>
      ...
      <ScrollView style={styles.listArea}>
        <Items
          key={`forceupdate-donee-${forceUpdateId}`}
          done={false}
          onPressItem={(id) =>
            db.transaction(
              (tx) => {
        ...
        <Items
          ...
          onPressItem={(id) =>
            db.transaction(
              (tx) => {
                tx.executeSql(`delete from items where idd = ?;`, [id]);
              },
              null,
              forceUpdate
            )
